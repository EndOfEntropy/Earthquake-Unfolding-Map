...
public class EarthquakeCityMap extends PApplet {
	...
	private void addKey() {	
		...
		int x = 60, y = 100, size = 10;
		fill(color(100, 0, 0));
		triangle(x, y-5, x-5, y+5, x+5, y+5);
		
		fill(255, 255, 255);
		ellipse(x, y+20, size, size);
		rect(x-5, y-5+40, size, size);
		fill(yellow);
		ellipse(x, y+80, size, size);
		fill(blue);
		ellipse(x, y+100, size, size);
		fill(red);
		ellipse(x, y+120, size, size);
		fill(255, 255, 255);
		ellipse(x, y+140, size, size);
		
		fill(0, 0, 0);
		
		text("City Marker", x+20, y);
		text("Land Quake", x+20, y+20);
		text("Ocean Quake", x+20, y+40);
		text("Size ~ Magnitude", 50, y+60);
		text("Shallow", x+20, y+80);
		text("Intermediate", x+20, y+100);
		text("Deep", x+20, y+120);
		text("Past Hour", x+20, y+140);
	...
	private void printQuakes() 
	{
		...
		for (Marker cm : countryMarkers) {
			...
			for(Marker qm: quakeMarkers) {
				...
				if(em.isOnLand()) {
					String nameOfcm = (String)cm.getProperty("name");
					String countryOfqm = (String)qm.getProperty("country");
					if(nameOfcm.contains(countryOfqm)) {
						quakeCounter++;
					}
				} else {
					oceanCounter++;
			...
			if(quakeCounter > 0) {
				System.out.println((String)cm.getProperty("name") + " has " + quakeCounter + " earthquakes.");
			}
			count++;
		}
		System.out.println("Ocean has " + oceanCounter/count + " quakes");
